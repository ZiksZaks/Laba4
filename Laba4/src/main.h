...
typedef struct {
    float x, y, dx, dy;
    const float HERO_HEIGHT = 61.0f;
    const float HERO_WIDTH = 50.0f;
    bool isAirborne;
    bool isMoving;
    bool facingLeft;
} Hero;
...
void Sprite_animation(GLuint texture, int n, Hero *obj) {
    ...
    static float spriteXsize = 1000;
    static float spriteYsize = 250;
...
void Main_Init() {
    ...
    Load_Texture("img/Duck.png", &sprite, GL_CLAMP, GL_CLAMP, GL_NEAREST);
    Load_Texture("img/bg.jpg", &background, GL_REPEAT, GL_CLAMP, GL_NEAREST);
}

void Menu_Init() {
    Menu_AddButton("Play", 10.0, 10.0, 100.0, 30.0, 2);
    Menu_AddButton("Menu", 10.0, 50.0, 100.0, 30.0, 2);
    Menu_AddButton("Exit", 10.0, 90.0, 100.0, 30.0, 2);
}
...
void draw_map() {
    glColor3f(0.8f, 0.5f, 0.0f); // Оливковый цвет
